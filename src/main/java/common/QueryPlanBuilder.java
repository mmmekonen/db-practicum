...
public class QueryPlanBuilder {
  ...
  public LogicalOperator logicalPlan(Statement stmt) {
    ...
    SelectUF conditions = null;
    if (where != null) conditions = new SelectUF(where);
    // System.out.println(conditions);

    // make logical query plan
    LogicalOperator rootOperator;
    if (joins == null) rootOperator = selectHelper(table, conditions);
    else rootOperator = joinHelper(table, joins, where, conditions);
  ...
  /**
   * Top level method to create physical plan from logical plan using a specified join type.
   *
  ...
  /**
   * Top level method to create physical plan from logical plan, determining join type from data
   * statistics.
   *
   * @param stmnt statement to be translated
   * @return the root of the query plan
   ...
   */
  private LogicalOperator selectHelper(Table table, SelectUF conditions) {
   ...
   */
  private LogicalOperator joinHelper(
      Table original, List<Join> joins, Expression where, SelectUF conditions) {
    ArrayList<LogicalOperator> children = new ArrayList<>();
    children.add(selectHelper(original, conditions));
    for (Join j : joins) children.add(selectHelper((Table) j.getRightItem(), conditions));
  ...
  /**
   * A method to create the string representation of the physical plan corresponding with a
   * statement
   *
   * @param stmnt The statement from which the plan will be created
