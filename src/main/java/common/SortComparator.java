...
/**
 * A class to represent a sort comparator on Tuples. Sorts each tuple based on the given sort order.
 */
public class SortComparator implements Comparator<Tuple> {
  ...
  /**
   * Creates a sort comparator that compares two tuples using the given sort order.
   *
   * @param sortOrder A HashMap of the final order position to the position of the element in the
  ...
  /**
   * Compares two tuples element-by-element. If one of the tuples are null, the other tuple is
   * considered to be smaller. If both are null, they are equal.
   *
