...
public class JoinOperator extends Operator {
  ...
  /**
   * Joins two operators together according to an expression, producing a single operator that
   * returns the next valid combination of its children's tuples
   *
   * @param left_op One of the operators to be joined
   * @param right_op One of the operators to be joine
   * @param expression An expression that dictates which combinations of tuples from the children
   *     operators are valid
   * @return a JoinOperator object
   */
  public JoinOperator(Operator left_op, Operator right_op, Expression expression) {
    super(null);
    this.outputSchema = left_op.copyOutputSchema();
    this.outputSchema.addAll(right_op.copyOutputSchema());
    this.left = left_op;
    this.right = right_op;
  ...
  /**
   * Iterates through combinations of outputs from the children operators until it finds one that is
   * valid, then returns it as a combined tuple
   *
